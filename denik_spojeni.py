...
# -*- coding: utf-8 -*-
"""
Deník spojení – světlé rozhraní ve stylu Radio Propagation Monitoru.

Čistá pracovní verze:
- bez starých nepoužívaných oken,
- bez zpětně kompatibilních aliasů,
- bez databázových migrací pro staré verze,
- databáze se zakládá přímo v aktuální struktuře programu,
- kód je rozdělený do bloků: konstanty, styl, pomocné funkce, databáze,
  GUI okna a start programu.

Poznámka k databázi:
Tato verze už neobsahuje automatické ALTER TABLE migrace starších databází.
Pokud by se používal starý soubor denik.sqlite3 z historické verze, je lepší
si jej předem zálohovat a založit novou databázi v aktuálním formátu.
"""
...
# Výchozí seznam kvalit spojení.
# Nově se ukládají do databázové tabulky quality_values,
# aby šly upravovat z menu a používat v comboboxu záznamu.
DEFAULT_QUALITY_VALUES = [(name,) for name in QUALITY_VALUES]

# Výchozí seznam výkonů. Hodnoty jsou uložené v databázové tabulce power_values
# a lze je upravovat z menu Nastavení.
DEFAULT_POWER_VALUES = [(name,) for name in ["0.5 W", "1 W", "2 W", "4 W", "5 W", "8 W", "10 W"]]

# Oddělené výchozí seznamy podle režimu záznamu.
# Analogový a digitální provoz používají odlišné typy spojení i modulace,
# proto se od této verze nečtou z jednoho společného seznamu.
...
# Výchozí digitální kanály. Hodnoty vycházejí ze struktury v ukázkové tabulce DMR.
# Uživatel je může libovolně upravit v menu Nastavení -> Nastavení digitálních kanálů.
DEFAULT_DIGITAL_CHANNELS = [
    # Název kanálu, frekvence, TG, CC, volací skupina, provoz, jméno, ID, kontakt
]

# Výchozí seznam CTCSS tónů a DCS kódů.
# Tabulka v databázi se při prvním spuštění automaticky naplní těmito hodnotami.
...
# ---------------------------------------------------------------------------
# VIZUÁLNÍ STYL APLIKACE - POUZE SVĚTLÝ REŽIM
# ---------------------------------------------------------------------------
# Tato část nahrazuje předchozí systém více motivů.
# Uživatel si ověřil, že světlý režim je pro PMR deník nejlepší,
# proto zde zůstává jediná pevná paleta.
#
# Důležitá myšlenka:
# - v programu už není dialog pro výběr motivu,
# - v menu Nastavení už není položka „Motiv“,
# - do databáze se už neukládá žádná hodnota theme,
# - všechna okna i dialogy používají jednu společnou paletu THEME.
#
# Výhoda proti více motivům:
# - menší a přehlednější zdrojový kód,
# - žádné riziko, že některé okno zůstane omylem v jiném motivu,
# - stabilní vzhled programu při každém spuštění,
# - jednodušší pozdější ruční úpravy barev.

THEME = {
    # Základní pozadí celé aplikace. Používá se na hlavní okno,
    # běžné rámečky a většinu ploch, které nejsou samostatné bílé karty.
    "bg": "#eef3f7",

    # Hlavní barva textu na světlém pozadí.
    "fg": "#111111",

    # Vedlejší, méně výrazný text. Používá se například pro podtitul
    # v horní části okna a pro stavový řádek dole.
    "sub": "#444444",

    # Barva panelů a „karet“. V tomto programu jsou panely bílé,
    # aby tabulka, formuláře a detailní okna působily čistě.
    "panel": "#ffffff",

    # Text uvnitř bílých panelů.
    "panel_fg": "#111111",

    # Základní barva tlačítek.
    "button": "#dde4ec",

    # Barva tlačítka při najetí myší.
    "button_hover": "#cfd8e3",

    # Barva okrajů rámečků, tabulek, scrollbarů a vstupů.
    "border": "#b8c6d4",

    # Hlavní akcent. Používá se pro důležitá tlačítka
    # a pro zvýraznění podobně jako v Radio Propagation Monitoru.
    "accent": "#146c2e",

    # Tmavší varianta akcentu pro stisknutý/aktivní stav.
    "accent2": "#0b8a3a",

    # Barva záhlaví tabulky.
    "table_header": "#e7eef6",

    # Běžný řádek tabulky.
    "table_row": "#ffffff",

    # Střídavý řádek tabulky. Lehký rozdíl pomáhá čitelnosti.
    "table_alt": "#f3f7fb",

    # Barva vybraného řádku v tabulce.
    "select": "#d7e9ff",

    # Text ve vybraném řádku.
    "select_fg": "#111111",

    # Pozadí vstupních polí a rozbalovacích seznamů.
    "entry_bg": "#ffffff",

    # Pozadí vypnutých prvků.
    "disabled_bg": "#e0e0e0",

    # Text vypnutých prvků.
    "disabled_fg": "#777777",
}
...
def apply_light_style(root):
    """
    Nastaví jednotný světlý vzhled celé Tkinter aplikace.

    Funkce se volá při startu programu ještě před vytvořením hlavního okna
    aplikace a znovu už se nepřepíná, protože v této verzi existuje pouze
    světlý režim.

    Tkinter má dva typy widgetů:
    1) klasické tk widgety, například tk.Label, tk.Text, tk.Canvas,
    2) modernější ttk widgety, například ttk.Button, ttk.Frame, ttk.Treeview.

    Klasické tk widgety se často barví přímo přes configure(bg=...).
    Ttk widgety se barví přes objekt ttk.Style. Proto je zde nastavení
    centralizované na jednom místě, aby se vzhled nemusel řešit v každém
    okně zvlášť.
    """
    # Barva hlavního okna. Tato barva je vidět všude tam, kde není
    # položený jiný rámec nebo widget.
    root.configure(bg=THEME["bg"])

    # ttk.Style řídí vzhled všech ttk prvků v aplikaci.
    style = ttk.Style(root)

    # Motiv „clam“ je v Tkinteru dobrý základ pro vlastní barvení.
    # Některé systémové motivy ignorují část barev, proto se snažíme
    # použít clam, pokud je dostupný.
    try:
        if "clam" in style.theme_names():
            style.theme_use("clam")
    except Exception:
        # Pokud by prostředí motiv nepodporovalo, aplikace poběží dál
        # se systémovým motivem. Funkčnost programu tím není ohrožena.
        pass

    # Společné fonty. Jsou definované zde, aby se dal celý program
    # později snadno zvětšit/zmenšit jen změnou několika řádků.
    default_font = ("Ubuntu", 10)
    title_font = ("Ubuntu", 18, "bold")
    bold_font = ("Ubuntu", 10, "bold")

    # Obecný styl pro všechny ttk widgety.
    style.configure(".", font=default_font, background=THEME["bg"], foreground=THEME["fg"])

    # Rámečky nemají vlastní okraj, jen přebírají pozadí aplikace.
    style.configure("TFrame", background=THEME["bg"])

    # Běžné popisky.
    style.configure("TLabel", background=THEME["bg"], foreground=THEME["fg"])

    # Velký nadpis v hlavním okně.
    style.configure("Title.TLabel", background=THEME["bg"], foreground=THEME["fg"], font=title_font)

    # Podtitul a méně výrazné informační texty.
    style.configure("Subtitle.TLabel", background=THEME["bg"], foreground=THEME["sub"], font=("Ubuntu", 10))

    # Propagation Monitor UI: bílé karty bez klasického šedého vzhledu.
    ...
    # LabelFrame je rámeček s titulkem. Používá se pro blok Stanice,
    # Filtry a další skupiny ovládacích prvků.
    style.configure(
        "TLabelframe",
        background=THEME["bg"],
        foreground=THEME["fg"],
    ...
    # Zvýrazněné tlačítko pro hlavní akci, například „Vytvořit záznam“.
    style.configure(
        "Accent.TButton",
    ...
    # Tabulka záznamů. Treeview má zvláštní nastavení pro řádky,
    # záhlaví a vybraný řádek.
    style.configure(
        "Treeview",
    ...
    # Scrollbary pro tabulku a další posuvné prvky.
    ...
    # Option database ovlivňuje klasické tk.Menu a rozbalovací seznamy
    # uvnitř comboboxu. Tyto prvky nejdou spolehlivě obarvit jen přes ttk.Style.
    try:
        ...
        # Kdyby některé prostředí option_add nepřijalo, není to kritická chyba.
        pass


def norm_loc(s):
    """
    Normalizuje Maidenhead lokátor zadaný uživatelem.

    Uživatel může napsat například "jn99 ao" nebo " JN99ao ".
    Program z toho udělá jednotný tvar "JN99AO".
    Díky tomu se v databázi nebudou míchat stejné lokátory v různém zápisu.
    """
...
def norm_call(s):
    """
    Normalizuje volací značku nebo označení stanice.

    Pro PMR se nemusí vždy jednat o oficiální radioamatérskou značku.
    Přesto je praktické ukládat hodnotu jednotně velkými písmeny.
    """
...
def db_connect():
    """
    Otevře lokální databázi SQLite a vytvoří aktuální tabulky programu.

    Tato funkce už neprovádí žádnou zpětnou kompatibilitu ani migrace
    starých databází. Program počítá s aktuální strukturou tabulek.

    Vytvářené tabulky:
    - logs: hlavní deník spojení,
    - settings: jednoduché nastavení uživatele,
    - equipment: seznam zařízení a antén,
    - channels: analogové kanály,
    - tone_codes: CTCSS/DCS kódy,
    - quality_values: editovatelný seznam kvalit,
    - power_values: editovatelný seznam výkonů,
    - operation_types_analog / operation_types_digital: typy spojení podle režimu,
    - modulation_modes_analog / modulation_modes_digital: modulace podle režimu,
    - digital_channels: digitální kanály včetně DMR parametrů.
    """
    con = sqlite3.connect(DB_FILE)
    con.row_factory = sqlite3.Row

    # Hlavní tabulka deníku.
    # Obsahuje společná pole pro analogový i digitální provoz.
    # U analogového záznamu bývají digitální položky prázdné.
    # U digitálního záznamu bývá naopak prázdný CTCSS/DCS tón.
    con.execute("""
    ...
    # Nastavení uživatele, například moje volačka a domovský lokátor.
    con.execute("""
    ...
    # Seznam radiostanic a antén.
    # kind rozlišuje hodnoty "device" a "antenna".
    con.execute("""
    ...
    # Editovatelné seznamy používané v comboboxech formuláře.
    con.execute("""
    ...
    # Digitální kanály. Struktura odpovídá aktuálnímu požadavku:
    # název kanálu, frekvence, TG, CC, volací skupina, provoz, jméno, ID a kontakt.
    con.execute("""
    ...
    # Naplnění výchozích hodnot.
    # Vkládá se pouze tehdy, pokud je daná tabulka prázdná.
    # Nejde o migraci staré databáze, ale o inicializaci nové čisté databáze.
...
class CalendarPopup:
    """
    Jednoduchy vyskakovaci kalendar bez externich knihoven.
    - sipky pro predchozi/dalsi mesic
    - vyber roku
    - kliknuti na den vybere datum
    """
    def __init__(self, parent, target_var):
        ...
        # Nejprve vytvoříme obsah okna a necháme Tkinter okno skutečně vykreslit.
        # Teprve potom nastavíme grab_set(). Na některých Linux/Tk prostředích
        # jinak vznikalo prázdné okno kalendáře a chyba:
...
class DateTimeSelector:
    """
    Kompaktní výběr data a času pro filtr Od/Do ve stylu Propagation Monitor UI.
    Pole nejsou nalepená na okraj karty a jsou v jedné vodorovné ose.
    """
    def __init__(self, parent, label, x, y):
        ...
        # Kliknutí do pole data už kalendář neotvírá.
        # Datum se vybírá pouze tlačítkem s ikonou kalendáře vedle pole,
        # aby omylem nevyskakovalo okno při běžné práci s filtrem.
...
class App:
    ...
    - filtry,
    - tabulku záznamů,
    - otevírání dialogů pro nový záznam, detail, nastavení a mapu.

    V programu je záměrně použito jedno hlavní připojení k databázi
    self.con, které sdílejí i podřízená okna. Je to jednoduché a pro
    lokální desktopový deník dostatečné.
    """
    def __init__(self, root):
        # Uložíme odkaz na hlavní Tk okno, aby s ním mohly pracovat
        # metody třídy App i podřízená dialogová okna.
        self.root=root; self.root.title(APP_TITLE); self.root.geometry('1500x760'); self.root.minsize(1040,650)

        # Otevření databáze a výchozí nastavení řazení tabulky.
        # Výchozí řazení je podle datumu sestupně, aby poslední záznamy
        # byly nahoře.
        self.con=db_connect(); self.sort_col='dt'; self.sort_desc=True
        # Vzhled je od této verze pevně světlý.
        # Nečteme už žádný uložený motiv z databáze.
    ...
    def menu(self):
        ...
        Menu je záměrně jednoduché:
        - Soubor: import, export a ukončení,
        - Nastavení: uživatel, zařízení a kanály.

        Položka „Motiv“ zde už není. Program má od této verze pouze
        pevný světlý režim, takže uživatel nemusí nic vybírat.
        """
    ...
    def ui(self):
        """
        Hlavní okno ve stylu Propagation Monitor UI.

        Styl je postavený na Tkinter/ttk, ale místo klasických natěsnaných
        groupboxů používá bílé karty, větší vnitřní odsazení a přehledné
        bloky podobné Radio Propagation Monitoru.
        """
        PAD = 14
        main = ttk.Frame(self.root)
        main.place(x=0, y=0, relwidth=1, relheight=1)

        # Horní hlavička aplikace.
        ...
        # Datumy a časy mají vlastní odsazený řádek uvnitř karty.
        ...
        # Spodní stavový řádek a tlačítko tisku.
        #
        # Tlačítko Tisk pracuje vždy s aktuálním filtrem v hlavním okně.
        # To znamená, že se nevytiskne celá databáze, ale pouze záznamy,
        # které odpovídají vyhledávání, datu Od/Do, kanálu a typu provozu.
    ...
    def load_rows(self):
        ...
        if ch and ch!='vše':
            # ch je text typu: PMR 1 / 446.00625 MHz
            if ' / ' in ch:
    ...
    def filtered_rows_for_output(self):
        """
        Vrátí záznamy podle aktuálně nastaveného filtru.

        Tato metoda je oddělená od load_rows(), protože stejná pravidla
        potřebujeme použít i pro tisk. Díky tomu se tiskne přesně to,
        co uživatel právě vidí v tabulce po použití filtru.
        ...
        - kanál,
        - typ provozu,
        - aktuální řazení tabulky.

        Návratová hodnota:
        - seznam sqlite3.Row záznamů,
        - None, pokud je chyba ve filtru data/času.
        """
        where = []
        params = []
    ...
    def current_filter_description(self):
        """
        Vytvoří stručný textový popis aktuálního filtru pro tiskovou sestavu.

        Popis se zobrazuje v hlavičce tiskové stránky, aby bylo později jasné,
        podle čeho byl výpis vytištěn.
        """
        parts = []
    ...
    def build_print_html(self, rows):
        """
        Sestaví HTML stránku pro tisk vyfiltrovaných záznamů.

        Proč HTML:
        - funguje na Linuxu i Windows,
        - otevře se ve výchozím prohlížeči,
        - prohlížeč zobrazí standardní systémové tiskové okno,
        - není potřeba pevně řešit konkrétní tiskárnu v Pythonu,
        - uživatel může tisknout i do PDF tiskárny.

        Stránka obsahuje malý JavaScript window.print(), který po otevření
        automaticky vyvolá tiskové okno. Pokud by to některý prohlížeč blokoval,
        v horní části stránky je i tlačítko „Tisk“.
        """
        def esc(value):
...
<body>
    ...
    def print_filtered_rows(self):
        """
        Vytiskne aktuálně vyfiltrované záznamy.

        Postup:
        1) z databáze se načtou záznamy podle stejného filtru jako v tabulce,
        2) vytvoří se dočasný HTML soubor s tiskovou sestavou,
        3) soubor se otevře ve výchozím prohlížeči,
        4) prohlížeč otevře standardní tiskové okno systému.

        Toto řešení je použitelné na Windows i Linuxu. Na rozdíl od příkazů
        lp/lpr není vázané jen na unixový tiskový systém.
        """
...
class RecordWindow:
    def __init__(self,app,rid=None):
        ...
        self.win=tk.Toplevel(app.root); self.win.configure(bg=THEME['bg']); self.win.title('Nový záznam' if rid is None else 'Editace záznamu'); self.win.geometry('760x560'); self.win.transient(app.root); self.win.grab_set()
        self.ui(); self.load_combo(); self.defaults() if rid is None else self.load(rid)

    def ui(self):
        """
        Okno nového/editovaného záznamu.

        Rozložení se mění podle režimu:
        - Analogový: kanál + CTCSS/DCS, bez digitálních polí TG/CC/ID/kontakt.
        - Digitální: digitální kanál + TG/CC/skupina/ID/kontakt, bez CTCSS/DCS.
        """
        # Výchozí velikost okna záznamu.
        # Okno je vyšší, aby byla spodní tlačítka Uložit/Zavřít vždy viditelná.
        self.win.geometry('1020x820')
        self.win.minsize(1020, 820)

        main = ttk.Frame(self.win)
        main.place(x=0, y=0, relwidth=1, relheight=1)

        self.card = ttk.Frame(main, style='Card.TFrame')
        self.card.place(x=18, y=18, relwidth=1.0, width=-36, relheight=1.0, height=-36)
        ...
        # Pravý sloupec je posunutý mírně doprava, aby u polí Výkon/Signál/Anténa/Azimut
        # nebyl uříznutý levý okraj a popisky se nepřekrývaly se vstupními poli.
        ...
        self.close_button = ttk.Button(self.card, text='✖  Zavřít', command=self.win.destroy)
    ...
    def update_record_layout(self):
        """Přeskupí formulář podle analogového/digitálního režimu."""
        mode = self.record_mode.get().strip() or 'Analogový'
        y = 22
        gap = self.gap

        # skryj proměnlivé prvky
        ...
        # Tlačítko mapy je na stejné vodorovné lince jako pole Směr
        # a začíná hned napravo od něj.
    ...
    def format_digital_channel(self, row):
        # V comboboxu digitálních kanálů zobrazujeme pouze název kanálu.
        # Frekvence, TG, CC, volací skupina, DMR ID a kontakt se po výběru
        # automaticky doplní do samostatných polí formuláře.
        return row['name'] or ''
    ...
    def format_channel(self, row):
        # V comboboxu nezobrazujeme databazove ID.
        # Uživatel vidi jen prehledny text: PMR 1 / 446.00625 MHz
    ...
    def selected_channel_row(self):
        ...
        # Primarne hledame podle celeho zobrazeneho textu.
    ...
    def open_map(self):
        # Mapa z okna noveho/editovaneho zaznamu se otevre uvnitr programu,
        # ne v internetovem prohlizeci.
        ...
        try:
            # Overime platnost lokatoru a prepocitame vzdalenost/azimut.
        ...
        # Vytvorime docasny objekt ve stylu sqlite row, aby mapa pouzila
        # stejnou logiku jako detail zaznamu.
        row = {
    ...
    def save(self):
        ...
        self.con.commit(); self.app.load_rows(); self.win.destroy()
...
class DetailWindow:
    ...
    def ui(self):
        outer=ttk.Frame(self.win,padding=12); outer.pack(fill='both',expand=True)

        title=(self.r['other_call'] or self.r['locator'] or 'Protistanice').strip()
        ttk.Label(outer,text=f"Detail zaznamu: {title}",font=('TkDefaultFont',13,'bold')).pack(anchor='w',pady=(0,8))
        ...
        right=ttk.LabelFrame(main,text='Technika a poznamka',padding=10)
        ...
        ttk.Label(right,text='Poznamka:',font=('TkDefaultFont',9,'bold')).grid(row=len(fields_right),column=0,sticky='nw',pady=(10,3))
        ...
        b=ttk.Frame(outer); b.pack(fill='x',pady=12)
...
class LocatorMapWindow:
    """
    Mapa spojeni uvnitr programu.

    Primarne pouziva knihovnu tkintermapview:
        pip install tkintermapview

    Pokud knihovna neni nainstalovana, pouzije jednoduche nouzove
    Canvas vykresleni bez mapoveho podkladu.
    """
...
class EquipmentSettings:
    ...
    def ui(self):
        """Nastavení zařízení ve stylu Propagation Monitor UI."""
        self.win.geometry('900x390')
        self.win.minsize(900, 390)
        main = ttk.Frame(self.win)
        main.place(x=0, y=0, relwidth=1, relheight=1)
...
class DualSimpleListSettings:
    """
    Dvousloupcové nastavení seznamů podle režimu.

    Levá tabulka je pro analogový provoz, pravá tabulka pro digitální provoz.
    Používá se pro typy spojení a režimy modulace, protože analog a digitál
    mají jiné hodnoty a formulář záznamu má nabízet jen hodnoty příslušného režimu.
    """
...
class SimpleListSettings:
    """
    Jednoduché nastavení jednosloupcového seznamu.

    Používá se pro:
    - Typy spojení/provozu,
    - Kvalitu spojení.

    Logika je podobná nastavení kanálů:
    - hodnoty jsou uložené v samostatné databázové tabulce,
    - kliknutí na záhlaví řadí seznam,
    - dvojklikem lze upravit buňku,
    - lze přidat řádek, smazat řádek, uložit změny a doplnit výchozí hodnoty.
    """
...
class AnalogSettings:
    """Jedno společné okno pro analogové kanály a CTCSS/DCS kódy."""
    def __init__(self, app):
        self.app = app
        self.con = app.con
        self.win = tk.Toplevel(app.root)
    ...
    def ui(self):
        ...
        # Spodní tlačítka uvnitř karty jsou v obyčejném rámečku bez okraje.
        # Původně zde byl použit Card.TFrame, který kolem tlačítek kreslil další
        # vnitřní orámování. To vizuálně vytvářelo nevzhledné čáry mezi tlačítky.
        ...
        # Stejně také tlačítka u tabulky CTCSS/DCS nemají vlastní orámování.
        ...
        #bottom.place(x=14, rely=1.0, y=-82, relwidth=1.0, width=-28, height=42)
...
class DigitalChannelSettings:
    """Editovatelný seznam digitálních kanálů a údajů stanic podle DMR tabulky."""
    def __init__(self, app):
        self.app = app
        self.con = app.con
        self.win = tk.Toplevel(app.root)
        ...
        # Sloupec „Provoz“ v tabulce digitálních kanálů není volný text.
        # Hodnoty se berou pouze z tabulky operation_types_digital, kterou
        # uživatel upravuje v menu Nastavení -> Nastavení typů spojení.
        #
        # Díky tomu se v digitálních kanálech nemohou objevit překlepy nebo
        # libovolné vlastní texty. Stejné hodnoty se potom používají i ve
        # formuláři Nový/Editace záznamu.
    ...
    def load_digital_operation_values(self):
        """
        Načte povolené typy digitálního spojení z databáze.

        Zdroj dat:
            operation_types_digital

        Tato metoda je použita:
        - při otevření okna nastavení digitálních kanálů,
        - při editaci buňky „Provoz“,
        - při přidání nového řádku,
        - při ukládání kvůli kontrole platnosti hodnot.
        """
        self.digital_operation_values = [
            row["name"]
    ...
    def first_digital_operation_value(self):
        """Vrátí první dostupný digitální typ spojení pro nový řádek."""
    ...
    def add_row(self):
        ...
        # Pro nový digitální kanál se jako výchozí hodnota „Provoz“
        # použije první položka ze seznamu digitálních typů spojení.
        # Pokud je seznam prázdný, pole zůstane prázdné a uživatel musí
        # nejdříve doplnit typy spojení v odpovídajícím nastavení.
    ...
    def start_cell_edit(self, event=None):
        ...
        # Sloupec „Provoz“ je v self.columns na pozici operation.
        # Ten nesmí být editován volným textem. Místo Entry zde použijeme
        # readonly Combobox naplněný z tabulky operation_types_digital.
        ...
        # Důležité: u readonly Comboboxu ve sloupci „Provoz“ nesmíme ukládat
        # buňku při události FocusOut. Při rozbalení seznamu totiž Tkinter
        # dočasně přesune fokus z comboboxu na jeho vyskakovací seznam.
        # Kdybychom zde nechali FocusOut stejně jako u obyčejného Entry,
        # seznam by jen problikl a okamžitě by se zavřel.
        #
        # Proto se Combobox ukládá pouze po skutečném výběru položky
        # nebo po stisku Enter. Běžné textové buňky dál používají FocusOut,
        # aby se hodnota uložila i při kliknutí mimo buňku.
        if self.columns[idx] == "operation":
    ...
    def save_changes(self):
        ...
        try:
            for item in self.tree.get_children():
                ...
                # Kontrola sloupce „Provoz“.
                # Povolené jsou pouze položky ze seznamu digitálních typů spojení.
                # Tím je zajištěno, že digitální kanál nemůže obsahovat libovolný
                # ručně napsaný text mimo centrálně spravovaný seznam.
...
# ------------------------------------------------------------
def main():
    # Vytvoreni hlavniho Tkinter okna
    root=tk.Tk()
